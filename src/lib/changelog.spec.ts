...
describe("changelog", () => {
  it("shows the latest CI fix release before the consolidated 2.0.0 release", () => {
    ...
    expect(versions[0]).toBe("2.0.2")
    expect(versions[1]).toBe("2.0.1")
  ...
  it("returns the 2.0.2 changelog entry for the current version", () => {
    const release = currentVersionChangelog("2.0.2")[0]
